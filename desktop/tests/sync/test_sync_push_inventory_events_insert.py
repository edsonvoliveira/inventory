# desktop/tests/sync/test_sync_push_inventory_events_insert.py

"""
Responsibilities:
- Test sync push inventory events insert behavior.
"""

# desktop/tests/test_sync_push_inventory_events_insert.py

from desktop.core.http_client import post
from desktop.config.settings import SYNC_PUSH_ENDPOINT
import uuid

TEST_JWT = "eyJhbGciOiJIUzI1NiIsImtpZCI6IlNxVEg5QjRUS21LM3VJY3QiLCJ0eXAiOiJKV1QifQ..POyimlmWr0qbV0CgiP1BYt9aZAuSvo11SaLsCjpKzlE"
EVENT_UUID = str(uuid.uuid4())

def main():
    payload = {
        "items": [
            {
                "table_name": "inventory_events",
                "operation": "insert",
                "record_uuid": EVENT_UUID,
                "payload": {
                    "location_id": 1,
                    "title": "Inventário Geral Dezembro",
                    "event_type": "full",
                    "required_counts": 1,
                    "tolerance_percent": 1.5
                }
            }
        ]
    }

    print("Enviando payload:", payload)

    response = post(
        SYNC_PUSH_ENDPOINT,
        TEST_JWT,
        payload
    )

    print("Resposta do DV Server:", response)

if __name__ == "__main__":
    main()