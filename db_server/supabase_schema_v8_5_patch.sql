-- SUPABASE SCHEMA PATCH (V8.5)
-- Aplicar em ambiente existente (V8)
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'inventory_items' THEN
    SELECT z.event_id INTO v_event_id FROM public.zones z WHERE z.id = NEW.zone_id;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    BEFORE INSERT OR UPDATE ON public.inventory_event_targets
    FOR EACH ROW EXECUTE PROCEDURE public.block_when_event_finalized();
  END IF;
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    ...
    BEFORE INSERT OR UPDATE ON public.zone_user_progress
    FOR EACH ROW EXECUTE PROCEDURE public.block_when_event_finalized();
  END IF;
END$$;
