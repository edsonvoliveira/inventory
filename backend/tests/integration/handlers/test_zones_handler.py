...
def get_handler():
    mod = importlib.import_module("app.services.sync.handlers.zones")
    for obj in vars(mod).values():
        if isinstance(obj, type) and issubclass(obj, BaseSyncHandler):
            if getattr(obj, "table_name", None) == "zones":
                return obj()
    raise RuntimeError("Handler de zones não encontrado.")
...
def cleanup(uuid: str):
    sb = get_supabase_service_client()
    sb.table("zones").delete().eq("uuid", uuid).execute()


def test_zones_pull_bootstrap():
    handler = get_handler()
    data = handler.pull(company_id=TEST_COMPANY_ID, since=None)
    assert isinstance(data, list)


def test_zones_pull_incremental():
    handler = get_handler()
    sb = get_supabase_service_client()
...
def test_zones_push_insert_and_pull():
    handler = get_handler()
    record_uuid = str(uuid4())
    event_id = ensure_event_id()
...
def test_zones_push_update():
    ...
    user = FakeCurrentUser(company_server_id=TEST_COMPANY_ID, db_user_id=TEST_USER_ID)
    handler.update(payload={"name": "Zona Atualizada"}, record_uuid=record_uuid, user=user)

    try:
        resp = sb.table("zones").select("name").eq("uuid", record_uuid).execute()
        assert resp.data and resp.data[0]["name"] == "Zona Atualizada"
    finally:
        cleanup(record_uuid)


def test_zones_push_soft_delete():
    handler = get_handler()
    sb = get_supabase_service_client()
    record_uuid = str(uuid4())
    event_id = ensure_event_id()
    ...
    try:
        resp = sb.table("zones").select("is_active").eq("uuid", record_uuid).execute()
